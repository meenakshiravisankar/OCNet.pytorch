...
#  Training
########################################################################################################################
$PYTHON -u train.py --ignore-label $IGNORE_LABEL --num-classes $NUM_CLASSES --experiment-name $EXPERIMENT_NAME --network $NETWORK --method $METHOD --random-mirror --random-scale --gpu 0 --batch-size $BATCHSIZE \
